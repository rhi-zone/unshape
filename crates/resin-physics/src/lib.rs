...
/// Configuration for jiggle physics simulation.
...
pub struct JiggleConfig {
    ...
    pub damping: f32,
    /// Mass of the jiggle point.
    pub mass: f32,
    /// Gravity influence (0 = none, 1 = full).
    pub gravity_scale: f32,
    /// Maximum displacement from rest position.
    pub max_displacement: f32,
    /// Inertia scale (how much parent motion affects this point).
    pub inertia: f32,
}
...
impl JiggleConfig {
    /// Preset for soft, bouncy motion (like soft tissue).
    pub fn soft() -> Self {
        Self {
            stiffness: 50.0,
            damping: 3.0,
    ...
    /// Preset for stiff, quick motion (like ears or antennae).
    pub fn stiff() -> Self {
        Self {
            stiffness: 300.0,
            damping: 10.0,
...
/// A single jiggle point that oscillates around a target.
#[derive(Clone, Debug)]
pub struct JigglePoint {
    /// Current position.
    pub position: Vec3,
    /// Current velocity.
    pub velocity: Vec3,
    /// Rest/target position (in local space).
    pub rest_position: Vec3,
    /// Configuration.
    pub config: JiggleConfig,
}

impl JigglePoint {
    /// Creates a new jiggle point at the given rest position.
    pub fn new(rest_position: Vec3) -> Self {
        Self {
            position: rest_position,
            velocity: Vec3::ZERO,
            rest_position,
            config: JiggleConfig::default(),
        }
    }

    /// Creates a jiggle point with custom configuration.
    pub fn with_config(rest_position: Vec3, config: JiggleConfig) -> Self {
        Self {
            position: rest_position,
            velocity: Vec3::ZERO,
            rest_position,
    ...
    /// Updates the jiggle point given a new target position.
    ///
    /// # Arguments
    /// * `target` - New target/rest position in world space
    /// * `dt` - Time step
    /// * `gravity` - Gravity vector
    pub fn update(&mut self, target: Vec3, dt: f32, gravity: Vec3) {
        let cfg = &self.config;
        ...
        let spring_force = -displacement * cfg.stiffness;

        // Damping force: F = -c * v
        let damping_force = -self.velocity * cfg.damping;

        // Gravity force
        let gravity_force = gravity * cfg.mass * cfg.gravity_scale;

        // Total force
        let total_force = spring_force + damping_force + gravity_force;

        // Acceleration (F = ma)
        let acceleration = total_force / cfg.mass;
        ...
        // Clamp displacement
        let new_displacement = self.position - target;
        if new_displacement.length() > cfg.max_displacement {
            self.position = target + new_displacement.normalize() * cfg.max_displacement;
            // Reduce velocity when hitting limit
            self.velocity *= 0.5;
        }

        // Update rest position for next frame
        self.rest_position = target;
    }

    /// Resets the jiggle point to its rest position.
    pub fn reset(&mut self) {
        self.position = self.rest_position;
        self.velocity = Vec3::ZERO;
    }

    /// Gets the current offset from rest position.
    pub fn offset(&self) -> Vec3 {
        self.position - self.rest_position
    }
}

/// A chain of connected jiggle points (for hair, tails, etc.).
#[derive(Clone, Debug)]
pub struct JiggleChain {
    /// Points in the chain (first is root/anchor).
    pub points: Vec<JigglePoint>,
    /// Segment lengths between points.
    pub lengths: Vec<f32>,
    /// Base configuration (may vary per point).
    pub config: JiggleConfig,
}

impl JiggleChain {
    /// Creates a new jiggle chain from a series of positions.
    pub fn new(positions: &[Vec3], config: JiggleConfig) -> Self {
        let mut points = Vec::with_capacity(positions.len());
        let mut lengths = Vec::with_capacity(positions.len().saturating_sub(1));

        for (i, &pos) in positions.iter().enumerate() {
            // Vary config along chain (more flexible at tips)
            let mut point_config = config;
            let t = i as f32 / (positions.len() - 1).max(1) as f32;
            point_config.stiffness *= 1.0 - t * 0.5; // Softer at tips
            point_config.damping *= 1.0 - t * 0.3;

            points.push(JigglePoint::with_config(pos, point_config));

            if i > 0 {
                lengths.push((positions[i] - positions[i - 1]).length());
            }
        }
    ...
    /// Creates a chain along an axis with specified segment count and length.
    pub fn along_axis(
        start: Vec3,
        direction: Vec3,
        segment_length: f32,
        ...
        let positions: Vec<Vec3> = (0..=segment_count)
            .map(|i| start + dir * segment_length * i as f32)
            .collect();
        Self::new(&positions, config)
    }

    /// Updates the chain with the root anchored at a new position.
    ///
    /// # Arguments
    /// * `root_position` - New position for the root/anchor point
    /// * `dt` - Time step
    /// * `gravity` - Gravity vector
    pub fn update(&mut self, root_position: Vec3, dt: f32, gravity: Vec3) {
        if self.points.is_empty() {
            return;
        }

        // Anchor the root
        self.points[0].position = root_position;
        self.points[0].rest_position = root_position;
        self.points[0].velocity = Vec3::ZERO;

        // Update each subsequent point
        for i in 1..self.points.len() {
            let parent_pos = self.points[i - 1].position;
            let target_length = self.lengths[i - 1];

            // Calculate target position (maintain distance from parent)
            let current_offset = self.points[i].position - parent_pos;
            let target = if current_offset.length() > 0.001 {
                parent_pos + current_offset.normalize() * target_length
            } else {
                parent_pos + Vec3::Y * target_length
            };

            // Update with spring physics
            self.points[i].update(target, dt, gravity);

            // Enforce distance constraint
            let to_point = self.points[i].position - parent_pos;
            let current_length = to_point.length();
            if current_length > 0.001 {
                let correction = (current_length - target_length) / current_length;
                self.points[i].position -= to_point * correction;
            }
        }
    }

    /// Updates the chain with root position and rotation.
    pub fn update_with_rotation(
        ...
        // Transform the chain's initial direction by the rotation
        let initial_offset = if self.points.len() > 1 {
            (self.points[1].rest_position - self.points[0].rest_position).normalize()
        } else {
            Vec3::Y
        };

        let rotated_offset = root_rotation * initial_offset;

        // Update with inertia from rotation
        self.update(root_position, dt, gravity);

        // Apply rotational influence to nearby points
        let num_points = self.points.len() as f32;
        for (i, point) in self.points.iter_mut().enumerate().skip(1) {
            let t = 1.0 - (i as f32 / num_points);
            let rotation_influence = rotated_offset * t * point.config.inertia * 0.1;
            point.velocity += rotation_influence / dt.max(0.001);
        }
    }
    ...
    /// Gets positions of all points.
    pub fn positions(&self) -> Vec<Vec3> {
        self.points.iter().map(|p| p.position).collect()
    }

    /// Gets the tip position (last point).
...
impl JiggleMesh {
    ...
    pub fn new(positions: &[Vec3], config: JiggleConfig) -> Self {
        let points: Vec<JigglePoint> = positions
            .iter()
            .map(|&pos| JigglePoint::with_config(pos, config))
            .collect();
    ...
    ///
    /// # Arguments
    /// * `targets` - New target positions for each vertex
    /// * `dt` - Time step
    /// * `gravity` - Gravity vector
    pub fn update(&mut self, targets: &[Vec3], dt: f32, gravity: Vec3) {
        for (i, (point, &target)) in self.points.iter_mut().zip(targets.iter()).enumerate() {
            if self.anchored[i] {
                point.position = target;
                point.rest_position = target;
                point.velocity = Vec3::ZERO;
            } else {
                point.update(target, dt, gravity);
            }
        }
    }
    ...
    pub fn update_with_transform(
        ...
        let targets: Vec<Vec3> = self
            .rest_positions
            .iter()
            .map(|&pos| translation + rotation * pos)
            .collect();
        self.update(&targets, dt, gravity);
    }

    /// Resets all points to their rest positions.
    pub fn reset(&mut self) {
        for (point, &rest) in self.points.iter_mut().zip(self.rest_positions.iter()) {
            point.position = rest;
            point.velocity = Vec3::ZERO;
            point.rest_position = rest;
        }
    }
...
impl WindForce {
    ...
    /// Advances time and returns the current wind vector.
    pub fn sample(&mut self, dt: f32, position: Vec3) -> Vec3 {
        ...
        let turbulence_offset = Vec3::new(noise_x, noise_y, noise_z) * self.turbulence;

        self.direction + turbulence_offset
    }
}
...
mod tests {
    ...
    // ========================================================================
    // Jiggle Physics tests
    // ========================================================================

    #[test]
    fn test_jiggle_config_default() {
        let config = JiggleConfig::default();
    ...
    #[test]
    fn test_jiggle_config_presets() {
        let soft = JiggleConfig::soft();
        let stiff = JiggleConfig::stiff();

        assert!(soft.stiffness < stiff.stiffness);
        assert!(soft.damping < stiff.damping);
    }

    #[test]
    fn test_jiggle_point_creation() {
        let point = JigglePoint::new(Vec3::new(1.0, 2.0, 3.0));
        assert_eq!(point.position, Vec3::new(1.0, 2.0, 3.0));
        assert_eq!(point.velocity, Vec3::ZERO);
        assert_eq!(point.rest_position, Vec3::new(1.0, 2.0, 3.0));
    }

    #[test]
    fn test_jiggle_point_update() {
        let mut point = JigglePoint::new(Vec3::ZERO);

        // Move target away - point should start moving towards it
        let target = Vec3::new(0.1, 0.0, 0.0);
        let gravity = Vec3::new(0.0, -9.81, 0.0);

        for _ in 0..100 {
            point.update(target, 1.0 / 60.0, gravity);
        }

        // Point should have moved and have some velocity
        assert!(point.position.length() > 0.0);
    }

    #[test]
    fn test_jiggle_point_reset() {
        let mut point = JigglePoint::new(Vec3::ZERO);
        point.position = Vec3::ONE;
        point.velocity = Vec3::ONE;

        point.reset();

        assert_eq!(point.position, point.rest_position);
        assert_eq!(point.velocity, Vec3::ZERO);
    }

    #[test]
    fn test_jiggle_point_max_displacement() {
        let mut point = JigglePoint::with_config(
            Vec3::ZERO,
            JiggleConfig {
                max_displacement: 0.1,
                stiffness: 10.0,
        ...
        // Apply large impulse
        point.velocity = Vec3::new(100.0, 0.0, 0.0);

        for _ in 0..10 {
            point.update(Vec3::ZERO, 1.0 / 60.0, Vec3::ZERO);
        }

        // Should be clamped to max displacement
        assert!(point.offset().length() <= 0.1 + 0.01);
    }

    #[test]
    fn test_jiggle_chain_creation() {
        let positions = vec![Vec3::ZERO, Vec3::Y, Vec3::Y * 2.0];
        let chain = JiggleChain::new(&positions, JiggleConfig::default());

        assert_eq!(chain.points.len(), 3);
        assert_eq!(chain.lengths.len(), 2);
        assert!((chain.lengths[0] - 1.0).abs() < 0.001);
    }

    #[test]
    fn test_jiggle_chain_along_axis() {
        let chain = JiggleChain::along_axis(Vec3::ZERO, Vec3::Y, 0.5, 4, JiggleConfig::tail());

        assert_eq!(chain.points.len(), 5); // 4 segments = 5 points
        assert_eq!(chain.lengths.len(), 4);
    }

    #[test]
    fn test_jiggle_chain_update() {
        let mut chain =
            JiggleChain::along_axis(Vec3::ZERO, -Vec3::Y, 0.3, 3, JiggleConfig::default());

        let gravity = Vec3::new(0.0, -9.81, 0.0);

        // Move root and simulate
        for i in 0..60 {
            let root_pos = Vec3::new((i as f32 * 0.1).sin() * 0.5, 0.0, 0.0);
            chain.update(root_pos, 1.0 / 60.0, gravity);
        }

        // Root should be at last position
        let last_root = Vec3::new((59.0_f32 * 0.1).sin() * 0.5, 0.0, 0.0);
        assert!((chain.points[0].position - last_root).length() < 0.01);

        // Tip should have moved (secondary motion)
        assert!(chain.tip().is_some());
    }

    #[test]
    fn test_jiggle_chain_positions() {
        let chain = JiggleChain::along_axis(Vec3::ZERO, Vec3::Y, 1.0, 2, JiggleConfig::default());
    ...
    fn test_jiggle_mesh_creation() {
        ...
        let mesh = JiggleMesh::new(&positions, JiggleConfig::soft());

        assert_eq!(mesh.points.len(), 3);
        assert_eq!(mesh.rest_positions.len(), 3);
        assert!(mesh.anchored.iter().all(|&a| !a));
    }

    #[test]
    fn test_jiggle_mesh_anchor() {
        let positions = vec![Vec3::ZERO, Vec3::X, Vec3::Y];
        let mut mesh = JiggleMesh::new(&positions, JiggleConfig::default());

        mesh.anchor_vertices(&[0, 2]);
    ...
    #[test]
    fn test_jiggle_mesh_update() {
        let positions = vec![Vec3::ZERO, Vec3::X, Vec3::Y];
        let mut mesh = JiggleMesh::new(&positions, JiggleConfig::default());
        mesh.anchor_vertices(&[0]);

        let targets = vec![Vec3::ZERO, Vec3::X * 1.1, Vec3::Y * 1.1];
        let gravity = Vec3::new(0.0, -9.81, 0.0);

        for _ in 0..30 {
            mesh.update(&targets, 1.0 / 60.0, gravity);
        }

        // Anchored point should stay at target
        assert_eq!(mesh.points[0].position, Vec3::ZERO);

        // Other points should have moved
        assert!(mesh.points[1].position != positions[1]);
    }
    ...
    fn test_jiggle_mesh_reset() {
        ...
        // Modify positions
        mesh.points[0].position = Vec3::ONE;
        mesh.points[1].velocity = Vec3::ONE;

        mesh.reset();

        assert_eq!(mesh.points[0].position, Vec3::ZERO);
        assert_eq!(mesh.points[1].velocity, Vec3::ZERO);
    }
    ...
    fn test_wind_force() {
        ...
        let force1 = wind.sample(0.1, Vec3::ZERO);
        let force2 = wind.sample(0.1, Vec3::ZERO);

        // Force should vary over time
        assert_ne!(force1, force2);

        // Base direction should be roughly maintained
        assert!(force1.x > 0.0);
    }

    #[test]
    fn test_wind_force_position_variation() {
        let mut wind = WindForce::new(Vec3::X, 1.0);

        let force_a = wind.sample(0.0, Vec3::ZERO);
        let force_b = wind.sample(0.0, Vec3::new(10.0, 0.0, 0.0));

        // Different positions should give different forces
        assert_ne!(force_a, force_b);
    }
}
