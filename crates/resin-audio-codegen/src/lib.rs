...
//!
//! ```ignore
//! use rhizome_resin_audio_codegen::{SerialAudioGraph, SerialAudioNode, SerialParamWire, generate_effect};
//!
//! fn main() {
...
impl SerialAudioNode {
    ...
    fn field_type(&self) -> &'static str {
        match self {
            SerialAudioNode::Lfo { .. } => "rhizome_resin_audio::primitive::PhaseOsc",
            SerialAudioNode::Gain { .. } => "f32",
            SerialAudioNode::Delay { .. } => "rhizome_resin_audio::primitive::DelayLine<true>",
            SerialAudioNode::Mix { .. } => "f32",
            SerialAudioNode::Envelope { .. } => "rhizome_resin_audio::primitive::EnvelopeFollower",
            SerialAudioNode::Allpass { .. } => "rhizome_resin_audio::primitive::Allpass1",
    ...
    fn init_code(&self, sample_rate: &str) -> String {
        match self {
            ...
            SerialAudioNode::Envelope { attack, release } => {
                format!(
                    "rhizome_resin_audio::primitive::EnvelopeFollower::new({sample_rate}, {attack}, {release})"
                )
            }
            SerialAudioNode::Allpass { coefficient } => {
                format!("rhizome_resin_audio::primitive::Allpass1::new({coefficient})")
            }
...
///
/// This includes the necessary `use` statements for the generated code to compile.
...
///
/// Use this when generating a single effect in isolation.
/// For multiple effects in one file, use [`generate_header`] once followed
/// by multiple [`generate_effect`] calls.
...
fn generate_struct(graph: &SerialAudioGraph, name: &str, analysis: &GraphAnalysis) -> String {
    ...
    for (_idx, field_name) in &analysis.lfo_phase_incs {
        code.push_str(&format!("    {field_name}: f32,\n"));
    }

    // Add pre-computed modulation constants
    for (i, (_node_idx, _base, _scale)) in analysis.mod_constants.iter().enumerate() {
...
fn generate_audio_node_impl(
    ...
    } else if is_flanger_pattern(graph) {
        code.push_str(&generate_flanger_process(graph, analysis));
    } else {
...
fn is_tremolo_pattern(graph: &SerialAudioGraph) -> bool {
    ...
    has_lfo && has_gain && has_param_wire && no_delay && graph.nodes.len() <= 3
}
...
fn is_flanger_pattern(graph: &SerialAudioGraph) -> bool {
    let has_lfo = graph
        .nodes
        .iter()
        .any(|n| matches!(n, SerialAudioNode::Lfo { .. }));
...
fn generate_flanger_process(graph: &SerialAudioGraph, analysis: &GraphAnalysis) -> String {
    let mut code = String::new();
...
mod tests {
    ...
    fn test_tremolo_codegen() {
        ...
        assert!(code.contains("rhizome_resin_audio::primitive::PhaseOsc"));
