...
//!
//! ```ignore
//! use rhi_unshape_audio_codegen::{SerialAudioGraph, SerialAudioNode, SerialParamWire, generate_effect};
//!
//! fn main() {
...
impl SerialAudioNode {
    ...
    fn field_type(&self) -> &'static str {
        match self {
            SerialAudioNode::Lfo { .. } => "rhi_unshape_audio::primitive::PhaseOsc",
            SerialAudioNode::Gain { .. } => "f32",
            SerialAudioNode::Delay { .. } => "rhi_unshape_audio::primitive::DelayLine<true>",
            SerialAudioNode::Mix { .. } => "f32",
            SerialAudioNode::Envelope { .. } => "rhi_unshape_audio::primitive::EnvelopeFollower",
            SerialAudioNode::Allpass { .. } => "rhi_unshape_audio::primitive::Allpass1",
    ...
    fn init_code(&self, sample_rate: &str) -> String {
        match self {
            SerialAudioNode::Lfo { .. } => {
                "rhi_unshape_audio::primitive::PhaseOsc::new()".to_string()
            }
            SerialAudioNode::Gain { gain } => format!("{gain}_f32"),
            SerialAudioNode::Delay { max_samples, .. } => {
                format!("rhi_unshape_audio::primitive::DelayLine::new({max_samples})")
            }
            SerialAudioNode::Mix { mix } => format!("{mix}_f32"),
            SerialAudioNode::Envelope { attack, release } => {
                format!(
                    "rhi_unshape_audio::primitive::EnvelopeFollower::new({sample_rate}, {attack}, {release})"
                )
            }
            SerialAudioNode::Allpass { coefficient } => {
                format!("rhi_unshape_audio::primitive::Allpass1::new({coefficient})")
            }
...
pub fn generate_header() -> String {
    ...
    code.push_str("// Generated by rhi-unshape-audio-codegen\n");
    code.push_str("// Do not edit manually\n\n");
    code.push_str("use rhi_unshape_audio::graph::{AudioContext, AudioNode};\n\n");
    code
}
