...
- S3 Direct-manip primary but insufficient; precision is an axis — direct-manip default but approximate; exact (type/formula/numeric, unit/tempo-snap) co-equal on the same field; formula/code is a fallback. R.
