...
This document tracks inconsistencies across the unshape codebase and plans for normalization.
...
|-------|------|----------------|
| unshape-mesh | `Mat4` | Raw 4x4 matrix |
| unshape-motion | `Transform2D` | Struct with position, rotation, scale, anchor |
| unshape-rig | `Transform` | Struct with translation: Vec3, rotation: Quat, scale: Vec3 |
...
**Solution:**
- Added `Lerp` trait to `unshape-easing` (chosen over `unshape-core` to respect SRP)
- Implemented for: `f32`, `f64`, `Vec2`, `Vec3`, `Vec4`, `Quat`, `[T; N]`
- `unshape-rig` now uses `unshape-easing::Lerp` with `Interpolate` as supertrait (`Lerp + Clone + Default`)
- `unshape-color` implements `Lerp` for `LinearRgb`, `Hsl`, `Hsv`, `Rgba`

```rust
// In unshape-easing/src/lib.rs
pub trait Lerp {
...
- `unshape-easing`: `f32`, `f64`, `Vec2`, `Vec3`, `Vec4`, `Quat`, `[T; N]`
- `unshape-rig`: `Transform`
- `unshape-color`: `LinearRgb`, `Hsl`, `Hsv`, `Rgba`
- `unshape-motion`: `Transform2D`, `LinearTransform2D`

---

### 3. Duplicated Cubic Bezier 🟢

**Problem:** Identical cubic bezier evaluation implemented 3 times in unshape-vector.

**Solution:**
- Created `crates/unshape-vector/src/bezier.rs` with shared bezier utilities
...
| `unshape-color` types | `Rgba`, `LinearRgb`, `Hsl` structs |

**Solution:**
- Added `From` conversions in `unshape-color`:
...
|-------|-----------|-------|
| unshape-field | `sample(input: I, ctx: &EvalContext) -> O` | Generic trait |
| unshape-color | `sample(&self, t: f32) -> Rgba` | 1D gradient sampling |
| unshape-image | `sample_uv(&self, u: f32, v: f32) -> Rgba` | 2D, no context |
| unshape-rig | `sample(&self, time: f32) -> T` | Animation tracks |

**Decision: Skip this normalization.**

Adding `Field` trait impls to `Gradient` or `AnimationTrack` would require adding `unshape-field` as a dependency, creating coupling for limited benefit:
...
- `unshape-image` already implements `Field` where composability matters

**Convention:** Domain-specific `sample()` methods are the preferred API. Add `Field` impls only when field composition is actually needed (as in unshape-image).
...
- `unshape-image`: `ImageFieldError`
- `unshape-audio`: `WavError`
- `unshape-mesh`: `ObjError`
- `unshape-vector`: `FontError`, `SvgParseError`
- `unshape-gltf`: `GltfError`
...
| Crate | Convention |
|-------|------------|
| unshape-image | Screen coords: (0,0) top-left, Y down |
| unshape-vector/gradient_mesh | Math coords: counterclockwise, (0,0) bottom-left implied |
| unshape-mesh | Right-handed, Y-up (Blender/glTF convention) |
...
1. ~~**Cubic bezier dedup**~~ ✅ - Done: `unshape-vector/src/bezier.rs`
2. ~~**Interpolate trait**~~ ✅ - Done: `unshape-easing::Lerp`, impls in rig and color
